<article <?php hybrid_attr('post'); ?>>
	<?php if (is_singular(get_post_type())) : // If viewing a single post. ?>
		<?php get_the_image(array('size' => 'large-16by9', 'split_content' => true, 'order' => array('featured', 'attachment'), 'before' => '<div class="entry-media">', 'after' => '</div>')); ?>
	...
	<?php else : // If not viewing a single post. ?>
		<?php $post_thumbnail = get_the_image(array('size' => 'full', 'split_content' => true, 'order' => array('featured', 'attachment'), 'format' => 'array')); ?>
		<?php if($post_thumbnail): ?>
			<?php $attachment_id = attachment_url_to_postid($post_thumbnail['src']); ?>
	        <?php $attachment = get_post($attachment_id); ?>
			<?php $thumbnail = get_the_image(array('size' => 'small-16by9', 'split_content' => true, 'order' => array('featured', 'attachment'), 'format' => 'array')); ?>
			...
			<div <?php hybrid_attr('entry-media'); ?>>
				<div class="entry-media__inner has-action-buttons has-overlay">
		            ...
		            <div class="action-buttons action-buttons_top-right">
		                <a class="magnific-popup-image btn lightbox fadeIn" href="<?php echo $post_thumbnail['src']; ?>" title="<?php echo $attachment->post_excerpt; ?>">
		...
		<div class="entry-inner">
			<header class="entry-header">
				<?php if (is_sticky() && is_home() && !is_paged()): ?>
					<?php
                        printf(
                            '<div class="entry-byline"><span class="entry-sticky">%s</span></div><!-- .entry-byline -->',
                             __('Featured', 'twentyfifteen')
                         );
                    ?>
				<?php endif;?>
